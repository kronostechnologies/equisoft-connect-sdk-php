...
```php
createUser($databaseUuid, $usersCreateUserPayload): \Equisoft\SDK\EquisoftConnect\Model\UsersUser
```
...
$usersCreateUserPayload = new \Equisoft\SDK\EquisoftConnect\Model\UsersCreateUserPayload(); // \Equisoft\SDK\EquisoftConnect\Model\UsersCreateUserPayload

try {
    $result = $apiInstance->createUser($databaseUuid, $usersCreateUserPayload);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling DatabasesApi->createUser: ', $e->getMessage(), PHP_EOL;
}
```
...
| **usersCreateUserPayload** | [**\Equisoft\SDK\EquisoftConnect\Model\UsersCreateUserPayload**](../Model/UsersCreateUserPayload.md)|  | |
