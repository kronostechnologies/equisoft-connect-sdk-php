...
```php
legacyFinanceGetAccountStatement($contactId, $includeUnmanagedAssets): \Equisoft\SDK\EquisoftConnect\Model\LegacyFinanceGetAccountStatementResponse
```
...
try {
    $result = $apiInstance->legacyFinanceGetAccountStatement($contactId, $includeUnmanagedAssets);
    print_r($result);
} catch (Exception $e) {
